def mutations():
		...
		if t1 == "#" and t2 == "{}".format(cid):
			...
			while mut != "#" and k < (len(ft)-1):
				...
				if pos != "NOT_FOUND" and posw != "NOT_FOUND" and resm != "NOT_FOUND" and resw != "NOT_FOUND" and resm != "NA" and resw != "NA":
					...
					mut_res = resm[0]
					wt_res = resw[0]
					d[nmut] = (wt,mut,wt_pos,mut_pos,wt_res,mut_res)
					nmut = nmut + 1
				k = k + 1
				ft1 = ft[k].split()
				mut = ft1[0].strip("(,|',|\n")
	...
	for x in range(1,nmut):
		...
		t5 = d[x][4]
		t6 = d[x][5]
		g.write("{} {} {} {} {} {}".format(t1,t2,t3,t4,t5,t6))
		g.write("\n")
	g.close()
	return(list1)
...
def DSSP_ass():
	...
	f.write("#wt,mut,chwt,chmut,reswt,resmut,poswt,posmut,wt_acc,mut_acc")
	f.write("\n")
	...
	for y in range(1,nmut):
		...
		#count = 0
			...
			mut1 = pdb2
			mut_acc = t3
			f.write("{},{},{},{},{},{},{},{},{},{}".format(pdb1,c1,pdb2,c2,reswt,resmut,poswt,posmut,wt_acc,mut_acc))
			f.write("\n")
				
		except:
			print("#### ERROR ####")
			f.write("{},{},{},{},ERROR,ERROR,{},{},ERROR,ERROR".format(pdb1,c1,pdb2,c2,poswt,posmut))
			f.write("\n")
