...
def neighbours():
	...
	z1 = open("HETATM_{}".format(sys.argv[3]),"w")
	#temp = open("COM.txt","w")
			...
			try:
				...
				while k1 < len(c1):
					...
					if c2[0][0:1] == "H":
						...
						CM = COM(res,cd)
						resid_list1.append(resid)
						resname_list1.append(resname)
						com_list1.append(CM)
				...
				if count != 0:
					...
					while k3 < len(com_list1):
						...
						dis = (v1[0]-v2[0])**2 + (v1[1]-v2[1])**2 + (v1[2]-v2[2])**2
						if dis < (zdis*zdis):
							zres = zres + ",{}".format(resid_list1[k3])
							zresname = zresname +  ",{}".format(resname_list1[k3])
						k3 = k3 + 1
			
				z1.write("{}\n".format(pdbid))
				z1.write("{}".format(zres))
				z1.write("\n")
				z1.write("{}".format(zresname))
				z1.write("\n")
				k1 = k1 +1
			except:
